...
def get_policies(
    ...
    query = db.query(Policy).options(joinedload(Policy.provider))
...
def get_my_recommendations(
    ...
    recommendations = db.query(Recommendation).filter(Recommendation.user_id == current_user.id).options(
        joinedload(Recommendation.policy).joinedload(Policy.provider)
...
def get_my_claims(
    ...
    claims = db.query(Claim).options(joinedload(Claim.policy).joinedload(Policy.provider)).filter(
