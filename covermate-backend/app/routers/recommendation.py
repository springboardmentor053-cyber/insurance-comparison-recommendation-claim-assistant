...
# GET /recommendations — Risk-profile based recommendations
@router.get("/")
def get_recommendations(
    ...
    # Filter policies by risk profile premium range
    if risk == "Low":
        ...
        policies = db.query(Policy).filter(Policy.premium > 40000).all()
    else:
        policies = db.query(Policy).all()

    # Clear old recommendations for this user before saving new ones
    ...
    for policy in policies:
        ...
        # Base score for all risk levels (was only running for High before)
        if risk == "Low":
            score += 40
        ...
        # Premium-based bonus score (was nested inside High only — now fixed)
        if float(policy.premium) > 40000:
            score += 30
            reason_parts.append("Offers high financial coverage")
        elif float(policy.premium) > 15000:
            score += 20
        ...
        #  Deductible bonus (was stuck inside nested else — now fixed)
        if float(policy.deductible) < 10000:
            score += 20
        ...
        #  Term bonus (was stuck deep in nesting — now fixed)
...
#  POST /recommendations/generate — Real-time smart recommendation engine
...
def generate_recommendations(
    ...
    for policy in policies:
        ...
        # Filter out policies way above budget
        ...
        if data.health_status == "critical":
            ...
            if deductible < 10000:
                score += 10
                reason_parts.append("Deductible suitable for average health")
